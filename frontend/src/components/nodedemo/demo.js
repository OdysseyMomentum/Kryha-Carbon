...
function Map({ width, height, links: defaultLinks }) {
  ...
  const handleOnClickCircle = (node, i) => {
    ...
    if (i < 3) {
      setLinkz([{ s: i, d: linkz[0].d }, linkz[1]]);
    ...
    if (i > 5) {
      setLinkz([linkz[0], { s: linkz[1].s, d: i }]);
    }
  };
  ...
  return (
    <div>
      ...
      <div id="map" style={mapStyles}>
        ...
        >
          {linkz.map((link, i) => {
            console.log(yScale(nodes[link.s].y), xScale(nodes[link.s].x));
            return (
              <NodeLineLeft
                style={{ postition: "absolute", top: yScale(nodes[link.s].y), left: xScale(nodes[link.s].x) }}
              />
            );
          })}
          {
            //<div style={{ transform: "rotate(180deg)", postition: "absolute", top: 200, left: 200, color: "grey", width: "300px", height: "200px" }}>
            // </div>
            // <line
            // 	key={i}
